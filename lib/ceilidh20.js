...
function Ceilidh20(data, config) {
    ...
    if (typeof (config.genIVLen) != "number") {
        throw "The generate IV pair length value must be a number.";
    }

    // Make sure the value of generated IV length pair must not be less than or equal to 0.
    config.genIVLen = (Math.floor(config.genIVLen) <= 0) ? 32 : Math.floor(config.genIVLen);
