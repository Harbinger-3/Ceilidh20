...
function Ceilidh20(data, config) {
    ...
    if (typeof (config.genIVLen) != "number") {
        throw "The generate IV pair length must be defined and be a number.";
    }
    ...
    if (config.isEncrypt) { // Encryption
        // generate another random IV
        var generatedIV = uintArray(config.genIVLen, true);
        // Hash user-specified IV with generated IV to form a new XORed IV
        var newXORedIV = hexToBytes(Sha256.hash(toChars(generatedIV) + toChars(config.iv)));

        // encrypt plaintext with key, nonce, and new XORed IV
        var encryptedPart1 = Ceilidh20_main(isBufferSupported, stateVector, config, dataBytes, newXORedIV, 1);
        ...
        if (isBufferSupported) {
            var last32Bytes = dataBytes.slice(dataBytes.length - config.genIVLen); // buffer
        } else {
            var last32Bytes = arraySlice(dataBytes, -config.genIVLen); // regular array
        }

        // decrypt that encrypted IV with key and nonce
        var encryptedIV = Ceilidh20_main(isBufferSupported, stateVector, config, last32Bytes, null, 0);
        // Hash user-specified IV with generated IV to form a new XORed IV
        var newXORedIV = hexToBytes(Sha256.hash(toChars(encryptedIV) + toChars(config.iv)));
        ...
        encryptedData = Ceilidh20_main(isBufferSupported, stateVector, config, dataBytes, newXORedIV, 1);
    }
